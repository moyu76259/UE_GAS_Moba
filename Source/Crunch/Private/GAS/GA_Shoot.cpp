                                ...
                                const FGameplayAbilityActivationInfo ActivationInfo, const FGameplayEventData* TriggerEventData)
{
	...
	if(HasAuthorityOrPredictionKey(ActorInfo,&ActivationInfo))
	{
		...
		UAbilityTask_WaitGameplayEvent* WaitStopShootProjectileEvent = UAbilityTask_WaitGameplayEvent::WaitGameplayEvent(this, GetShootTag(), nullptr, false, false);
		WaitStopShootProjectileEvent->EventReceived.AddDynamic(this,&UGA_Shoot::ShootProjecttile);
		WaitStopShootProjectileEvent->ReadyForActivation();
	}
}
...
void UGA_Shoot::ShootProjecttile(FGameplayEventData Payload)
{
	...
	if(K2_HasAuthority())
	{
		...
		AProjectileActor* Projectile = GetWorld()->SpawnActor<AProjectileActor>(ProjectileClass, SocketLocation, OwnerAvatarActor->GetActorRotation(), SpawnParameters);
		if(Projectile)
		{
			Projectile->ShootProjectile(ShootProjectileSpeed, ShootProjectileRange, GetAimTargetIfValid(),
